...
if "messages" not in st.session_state:
    ...
    if PERSIST_CONVERSATIONS:
        _log_path = os.path.join("chat_logs", f"chat_history_{current_user}_latest.json")
...
if prompt := st.chat_input(
    ...
    with st.chat_message("assistant", avatar=bot_avatar):
        ...
        try:
            ...
            # Add confidence assessment reminder
            # TEMPORARILY DISABLED - The "MUST end with" instruction confuses Gemini when calling tools
            # TODO: Re-enable after fixing tool calling, or make it conditional (only add after tool results)
            confidence_reminder = """

[OPTIONAL] If providing a final answer (not calling tools), please include a confidence assessment."""
            # enhanced_system = enhanced_system + confidence_reminder  # Disabled for now
            ...
            if tool_calls:
                ...
                # IMPORTANT: Save full_resp (includes tool execution log) not current_response_content
